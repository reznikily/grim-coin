...
func (s *Server) Start(ctx context.Context, localIP string) error {
	log.Printf("Controller IP address: %s", localIP)
	...
	go func() {
		log.Printf("Starting observer WebSocket server on 0.0.0.0:8080")
		log.Printf("Open web/observer.html in browser to view network state")
...
func (s *Server) waitAndVerifyStates(txID string, expectedCount int) {
	...
	// Broadcasting updated network state
	s.broadcastNetworkState()
}
